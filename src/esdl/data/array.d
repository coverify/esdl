...
pure @system unittest
{
    ...
    for (size_t n = 0; n < 100; ++n)
    {
        ...
        // We check that a1 has the same length and contents as a0:
        {
            ...
            // but the compiler complains: "Error: incompatible types for `(a1.opSlice()) == (a0[])`: `RangeT!(Array!(float, "dummy"))` and `float[]`".
...
pure @system unittest
{
    auto arr = Array!(int, "dummy")(0, 2, 3);
    assert(arr[0] == 0);
    assert(arr.front == 0);
    assert(arr.back == 3);
...
{
    import std.algorithm.comparison : equal;
    auto arr = Array!(int, "dummy")(1, 2, 3);

    // concat
    auto b = Array!(int, "dummy")(11, 12, 13);
    arr ~= b;
    assert(arr.length == 6);
...
/// `Array!(bool, "dummy")` packs together values efficiently by allocating one bit per element
pure @system unittest
{
    auto arr = Array!(bool, "dummy")([true, true, false, true, false]);
    assert(arr.length == 5);
}
...
private struct RangeT(A)
{
    ...
    */
    ...
    /* E is different from T when A is more restrictively qualified than T:
       immutable(Array!(int, "dummy")) => T == int, E = immutable(int) */
...
@system unittest
{
    ...
    try
    {
        auto a = Array!(S, "dummy") (S(0), S(1), S(2), S(3));
...
@system unittest
{
    Array!(int, "dummy") a = Array!(int, "dummy")(1, 2, 3);
    //a._data._refCountedDebug = true;
    auto b = a.dup;
    assert(b == Array!(int, "dummy")(1, 2, 3));
    b.front = 42;
    assert(b == Array!(int, "dummy")(42, 2, 3));
    assert(a == Array!(int, "dummy")(1, 2, 3));
}

@system unittest
{
    auto a = Array!(int, "dummy")(1, 2, 3);
    assert(a.length == 3);
}

@system unittest
{
    const Array!(int, "dummy") a = [1, 2];
...
@safe unittest
{
    ...
    alias Heap = BinaryHeap!(Array!(int, "dummy"));
}
...
@system unittest
{
    ...
    auto b = Array!(int, "dummy")(11, 12, 13);
    auto c = a ~ b;
    assert(c == Array!(int, "dummy")(1, 2, 3, 11, 12, 13));
    assert(a ~ b[] == Array!(int, "dummy")(1, 2, 3, 11, 12, 13));
    assert(a ~ [4,5] == Array!(int, "dummy")(1,2,3,4,5));
}
...
@system unittest
{
    ...
    auto b = Array!(int, "dummy")(11, 12, 13);
    a ~= b;
    assert(a == Array!(int, "dummy")(1, 2, 3, 11, 12, 13));
}

@system unittest
{
    auto a = Array!(int, "dummy")(1, 2, 3, 4);
...
@system unittest
{
    auto a = Array!(int, "dummy")(1, 2, 3, 4, 5);
    auto r = a[2 .. a.length];
    assert(a.insertBefore(r, 42) == 1);
    assert(a == Array!(int, "dummy")(1, 2, 42, 3, 4, 5));
    r = a[2 .. 2];
    assert(a.insertBefore(r, [8, 9]) == 2);
    assert(a == Array!(int, "dummy")(1, 2, 8, 9, 42, 3, 4, 5));
}

@system unittest
{
    auto a = Array!(int, "dummy")(0, 1, 2, 3, 4, 5, 6, 7, 8);
    a.linearRemove(a[4 .. 6]);
    assert(a == Array!(int, "dummy")(0, 1, 2, 3, 6, 7, 8));
}
...
@system unittest
{
    ...
    auto a = Array!(int, "dummy")(0, 1, 2, 3, 4, 5, 6)[];
    auto b = Array!(int, "dummy")(6, 5, 4, 3, 2, 1, 0)[];
    alias A = typeof(a);
...
@system unittest
{
    ...
    auto a = Array!(int, "dummy")([0, 1, 2]); //Array
    a[] += 5;
    ...
    auto r = Array!(int, "dummy")([0, 1, 2])[]; //Array.Range
    r[] += 5;
...
@safe unittest
{
    ...
    alias A = Array!(S, "dummy");
    alias B = Array!(shared bool, "dummy")(shared bool);
}
...
@system unittest
{
    ...
    auto a = Array!(int, "dummy")([1, 2, 2].filter!"true"());
}

// https://issues.dlang.org/show_bug.cgi?id=8282
@safe unittest
{
    auto arr = new Array!(int, "dummy");
}
...
@safe unittest
{
    static assert(is(Array!(int, "dummy").Range));
    static assert(is(Array!(int, "dummy").ConstRange));
}
...
@system unittest
{
    ...
    auto array = Array!(NoDefaultCtor, "dummy")([NoDefaultCtor(1), NoDefaultCtor(2)]);
...
//     {
//         Array!(size_t, NAME).Payload _backend;
//         size_t _length;
//     }
...
//     /// ditto
//     bool opEquals(string RHS)(ref const Array!(T, RHS) rhs) const
//     {
//         if (empty) return rhs.empty;
//         if (rhs.empty) return false;
...
//      * $(BIGOH length + m), where `m` is the number of elements in `stuff`.
//      */
//     Array!(bool, NAME) opBinary(string op, Stuff)(Stuff rhs)
//     if (op == "~")
//     {
//         Array!(bool, NAME) result;
...
//      * Forwards to `insertBack`.
//      */
//     Array!(bool, NAME) opOpAssign(string op, Stuff)(Stuff stuff)
//     if (op == "~")
//     {
...
@system unittest
{
    ...
    auto a = Array!(bool, "dummy")([true, true, false, false, true, false]);
...
@system unittest
{
    ...
    auto a = Array!(bool, "dummy")(retro(arr));
...
@system unittest
{
    auto arr = Array!(bool, "dummy")([false, false, false, false]);
...
@system unittest
{
    ...
    auto arr = Array!(double, "dummy")(values);
}

@nogc @system unittest
{
    auto a = Array!(int, "dummy")(0, 1, 2);
    int[3] b = [3, 4, 5];
    short[3] ci = [0, 1, 0];
    auto c = Array!(short, "dummy")(ci);
    assert(Array!(int, "dummy")(0, 1, 2, 0, 1, 2) == a ~ a);
    assert(Array!(int, "dummy")(0, 1, 2, 3, 4, 5) == a ~ b);
    assert(Array!(int, "dummy")(0, 1, 2, 3) == a ~ 3);
    assert(Array!(int, "dummy")(0, 1, 2, 0, 1, 0) == a ~ c);
}

@nogc @system unittest
{
    auto a = Array!(char, "dummy")('a', 'b');
    assert(Array!(char, "dummy")("abc") == a ~ 'c');
    import std.utf : byCodeUnit;
    assert(Array!(char, "dummy")("abcd") == a ~ "cd".byCodeUnit);
}

@nogc @system unittest
{
    auto a = Array!(dchar, "dummy")("ąćę"d);
    assert(Array!(dchar, "dummy")("ąćęϢϖ"d) == a ~ "Ϣϖ"d);
    wchar x = 'Ϣ';
    assert(Array!(dchar, "dummy")("ąćęϢz"d) == a ~ x ~ 'z');
}
...
@system unittest
{
    auto a = Array!(bool, "dummy")([true, false, true, true]);
...
@system unittest
{
    import std.algorithm.comparison : equal;
    auto a = Array!(bool, "dummy")([true, false, true, true]);
    auto b = Array!(bool, "dummy")([true, true, false, true]);
    assert(equal((a ~ b)[],
...
@system unittest
{
    import std.algorithm.comparison : equal;
    auto a = Array!(bool, "dummy")([true, false, true, true]);
    auto b = Array!(bool, "dummy")([false, true, false, true, true]);
    a ~= b;
    assert(equal(
                a[],
...
@system unittest
{
    ...
    Array!(int, "dummy") arr = [1, 2, 4, 5];
    int[] data = arr.data();

    const Array!(int, "dummy") arr2 = [8, 9];
    ...
    Array!(int, "dummy") empty;
    assert(empty.data == []);
}
// }
//  else {
//    import std.container.array: stdArray = Array;
//    alias Array(T, string NAME) = stdArray!T;
//  }
