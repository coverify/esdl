    ...
    private struct Payload
    {
        ...
        if (isImplicitlyConvertible!(Elem, T))
        {
            ...
            if (_capacity == length)
            {
                ...
                bool overflow;
				size_t newCapacity = addu(capacity, capacity /* / 2 + 1 */, overflow);
                if (overflow)
                    assert(false, "Overflow");
			...
			if (SCRATCH || !is (T == struct)) {
			  auto oldLength = _payload.length;
			  _payload = _payload.ptr[0 .. _payload.length + 1];
			  _payload[oldLength] = elem;
			}
			else {
        ...
        if (isInputRange!Range && isImplicitlyConvertible!(ElementType!Range, T))
        {
			...
			static if (isRandomAccessRange!Range &&
					   (SCRATCH || !is (T == struct))) {
			  // use memcpy
			    _payload = _payload.ptr[0 .. _payload.length + r.length];
			    _payload[oldLength..oldLength+r.length] = r[];
				return r.length;
			}
			else {
			  size_t result;
